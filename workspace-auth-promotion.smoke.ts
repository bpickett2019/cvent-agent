...
const source = { id:"source", name:"Source", ownerJobId:"job", authScopeId:"document-a", eventId:"e712e34c-6117-4d13-bf4c-8ed54cf2b495", access:"readOnly", controller:"agent", status:"ready", createdAt:"", updatedAt:"", containerId:"c", providerSessionId:"session-source", apiUrl:"http://steel-source", viewerUrl:"http://viewer", error:null, activity:[] } as const;
const sibling = { ...source, id:"sibling", providerSessionId:"session-sibling", apiUrl:"http://steel-sibling" } as any;
const otherDocument = { ...source, id:"other-document", ownerJobId:"other-job", authScopeId:"document-b", providerSessionId:"session-other", apiUrl:"http://steel-other" } as any;
const activities:string[]=[]; const refreshed:string[]=[]; let saved:any;
const manager:any={get:async(id:string)=>id==='source'?source:null,list:async()=>[source,sibling,otherDocument],recordActivity:async(id:string,a:any)=>{activities.push(`${id}:${a.type}`);return source;},refreshAuthentication:async(id:string)=>{refreshed.push(id);return sibling;}};
...
assert.equal(refreshed.includes('other-document'),false,"a document login must never refresh another document's workspace");
