...
export function GoldenLogin() {
  ...
  return <>
    ...
    {viewer && <div className="modal-backdrop golden-login-modal"><section className="modal-card"><button className="modal-close" onClick={() => setViewer(null)}>×</button><span className="eyebrow">Golden Cvent login</span><h2>Authenticate once for every worker</h2><p>Complete Cvent login and MFA in the Steel viewer, then save the refreshed browser context.</p><iframe src={viewer} title="Steel Cvent login viewer" sandbox="allow-scripts allow-same-origin allow-forms allow-popups" /><div className="modal-actions"><a className="secondary-button" href={viewer} target="_blank" rel="noreferrer">Open full screen ↗</a><button className="approve-button" disabled={busy} onClick={() => void action("capture")}>Save refreshed login</button></div>{error && <p className="inline-error">{error}</p>}</section></div>}
  </>;
}
