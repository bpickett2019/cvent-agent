...
export function compileFullRRToEventSpec(seed: EventSpec, compiled: FullRRCompilation): EventSpec {
  ...
  const compiledLinks = legacyLinks.length ? legacyLinks : footer.filter((x) => x.outcome !== "unsupported").map((x) => clean({ key: x.key, label: x.sourceLabel, destination: x.destination, enabled: x.include, appliesToPaths: [] }));
  if (compiledLinks.length) spec.footer = { ...(spec.footer ?? { text: "", socialLinks: {}, links: [] }), links: compiledLinks };
  else delete spec.footer;
  ...
  const questionNames = questions.length ? questions.map((x) => x.internalName) : legacyQuestions.map((x) => x.internalName);
